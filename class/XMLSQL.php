...
class XMLSQL{
	...
	public function select($what = array('*')){
		if($this->_debug)
			$this->_log->message('Construction de la requête "SELECT"');
	...
	private function _prepareStmt(){
		switch($this->current_request){
			case self::SELECT:
				...
				break;
			case self::INSERT:
                return $this->_insertItem(null, null, $this->operation, $this->from, $this->position);
				break;
			case self::DELETE:
